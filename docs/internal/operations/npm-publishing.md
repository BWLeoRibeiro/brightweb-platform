...
2. Confirm the unscoped package name `create-bw-app` is available to publish.
3. Create an npm automation token.
4. Add the token to the repository secrets used by the release workflow.

## First-time npm setup

1. Log into npm with the account that owns the `@brightweblabs` scope and will publish `create-bw-app`.
...
5. Store that token in the repository secrets used by the release workflow.
...
5. GitHub Actions publishes the changed public packages, including `create-bw-app`, to npm.
