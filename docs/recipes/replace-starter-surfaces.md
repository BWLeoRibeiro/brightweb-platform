...
| `/crm` with `getCrmDashboardData()` | Starter CRM page glue | App-owned CRM UI built on the stable CRM helpers, loading only the needed slices such as `listCrmContacts()`, `listCrmOrganizations()`, `getCrmContactStatusStats()`, `listCrmStatusTimeline()`, `listCrmPrimaryContacts()`, and `listCrmOwnerOptions()` |
