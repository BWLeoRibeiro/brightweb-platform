...
export const dashboard: ProjectDashboardData = {
  ...
  tasks: [
    ...
    { id: "task-approval", projectId: "aurora", milestoneId: "milestone-build", title: "Validar integração editorial", description: "Aguardar credenciais do fornecedor.", status: "blocked", health: "off_track", priority: "urgent", assigneeProfileId: "owner-leo", assigneeLabel: "Leonel Ribeiro", reporterProfileId: "owner-leo", reporterLabel: "Leonel Ribeiro", dueDate: "2026-07-24", position: 1, blockedReason: "Credenciais externas em falta.", createdAt: now, updatedAt: now },
    { id: "task-wireframes", projectId: "aurora", milestoneId: "milestone-design", title: "Aprovar wireframes", description: null, status: "done", health: "on_track", priority: "low", assigneeProfileId: "member-ana", assigneeLabel: "Ana Matos", reporterProfileId: "owner-leo", reporterLabel: "Leonel Ribeiro", dueDate: "2026-06-20", position: 1, blockedReason: null, createdAt: now, updatedAt: now },
  ],
  links: [
...
export const mockProjectsClient: ProjectsUiClient = {
  ...
  async listProjects(params) { return listProjects(params); }, async getPortfolioStats() { return { total: 3, planned: 1, active: 1, atRisk: 1, overdue: 1 }; }, async getProjectDashboard() { return dashboard; }, async listProjectActivity() { return dashboard.activity; }, async listOrganizations() { return [{ id: "org-atlas", name: "Atlas Studio" }, { id: "org-north", name: "Northstar Foods" }]; }, async listAssignableProfiles() { return []; }, async createOrganization(input) { return { id: "org-preview", name: input.name }; }, async createProject(input) { return { id: "project-preview", name: input.name }; }, async updateProject() { return dashboard; }, async deleteProject() {}, async syncProjectMembers() { return dashboard; }, async createMilestone() { return dashboard; }, async updateMilestone() { return dashboard; }, async deleteMilestone() { return dashboard; },
  async createTask(_projectId, input) {
    dashboard.tasks = [...dashboard.tasks, { id: `task-preview-${dashboard.tasks.length + 1}`, projectId: dashboard.project.id, milestoneId: input.milestoneId ?? null, title: input.title, description: input.description ?? null, status: input.status ?? "todo", health: "on_track", priority: input.priority ?? "medium", assigneeProfileId: input.assigneeProfileId ?? null, assigneeLabel: dashboard.members.find((member) => member.profileId === input.assigneeProfileId)?.label ?? null, reporterProfileId: input.reporterProfileId ?? null, reporterLabel: null, dueDate: input.dueDate ?? null, position: dashboard.tasks.length + 1, blockedReason: input.blockedReason ?? null, createdAt: now, updatedAt: now }];
    return { ...dashboard, tasks: [...dashboard.tasks] };
  },
  async updateTask(_projectId, taskId, input) {
    dashboard.tasks = dashboard.tasks.map((task) => task.id === taskId ? { ...task, ...input, assigneeLabel: input.assigneeProfileId === undefined ? task.assigneeLabel : dashboard.members.find((member) => member.profileId === input.assigneeProfileId)?.label ?? null, updatedAt: now } : task);
    return { ...dashboard, tasks: [...dashboard.tasks] };
  },
  async deleteTask(_projectId, taskId) { dashboard.tasks = dashboard.tasks.filter((task) => task.id !== taskId); return { ...dashboard, tasks: [...dashboard.tasks] }; }, async createLink() { return dashboard; }, async updateLink() { return dashboard.links; }, async deleteLink() { return dashboard.links; },
};
