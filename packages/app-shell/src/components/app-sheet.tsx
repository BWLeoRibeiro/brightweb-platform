...
export const sheetSectionTitleClassName = "text-label font-semibold uppercase tracking-widest text-[color:var(--muted-foreground)]";
export const sheetFieldLabelClassName = "text-micro text-[length:var(--text-ui-fine)] font-semibold uppercase tracking-[var(--type-tracking-180)] text-[color:var(--muted-foreground)]";
...
export const sheetViewControlClassName = "h-7 appearance-none border-0 bg-transparent p-0 text-body shadow-none focus-visible:ring-0 disabled:cursor-default disabled:opacity-100";
...
export function AppSheetHeader({ icon: Icon, leading, eyebrow, title, description, aside, editing, className }: AppSheetHeaderProps) {
  return (
    <SheetHeader className={cn(sheetHeaderClassName, editing && sheetHeaderEditingClassName, className)}>
      {eyebrow ? <p className="mb-3 text-left text-micro text-[length:var(--text-ui-fine)] font-semibold uppercase tracking-[var(--type-tracking-160)] text-[color:var(--muted-foreground)]">{eyebrow}</p> : null}
      <div className="flex items-center gap-3.5">
        ...
        <div className="min-w-0 flex-1">
          <SheetTitle className="text-title text-foreground truncate leading-tight">{title}</SheetTitle>
          {description ? <SheetDescription className="text-meta text-muted-foreground mt-0.5">{description}</SheetDescription> : null}
        </div>
