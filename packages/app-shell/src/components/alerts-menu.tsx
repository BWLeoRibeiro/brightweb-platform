    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      ...
      <DropdownMenuContent align="end" sideOffset={16} collisionPadding={12} className="w-80 overflow-hidden rounded-[var(--radius-card)] p-0">
        ...
        <div className="flex max-h-96 min-h-[8rem] flex-col overflow-y-auto p-2xs">
          {loading ? <div className="space-y-3 px-sm py-xs"><SkeletonLine w="92%" /><SkeletonLine w="78%" /><SkeletonLine w="88%" /></div> : error ? <div className="text-meta text-muted-foreground px-sm py-md text-destructive">{error}</div> : notifications.length === 0 ? <div className="my-auto flex flex-col items-center gap-2xs px-sm py-lg text-center"><span className="mb-2xs flex size-9 items-center justify-center rounded-full bg-accent/10 text-accent"><Bell className="size-4" /></span><p className="text-micro text-muted-foreground text-foreground">Sem alertas novos</p><p className="text-meta text-muted-foreground">Está tudo em dia.</p></div> : notifications.map((notification) => <div key={notification.id} className="rounded-[var(--radius)] px-sm py-xs transition-colors hover:bg-accent/10"><div className="mb-2xs flex items-center justify-between gap-xs"><span className="text-micro text-muted-foreground text-accent">{notification.domain ?? "Sistema"}</span><span className="text-micro text-muted-foreground shrink-0 font-normal">{formatDate(notification.createdAt)}</span></div><p className="text-meta text-muted-foreground leading-snug text-foreground">{notification.summary}</p></div>)}
        </div>
      </DropdownMenuContent>
    </DropdownMenu>
  );
