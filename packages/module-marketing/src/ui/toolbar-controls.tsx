        ...
        <Popover open={open} onOpenChange={(next) => { setDraftStatus(status); setOpen(next); }}>
          <PopoverTrigger asChild>
            <Button type="button" variant="outline" disabled={!ready}>
              <Filter data-icon="inline-start" aria-hidden />Filtros
              {status !== "all" ? <span className="text-data-sm inline-flex size-5 items-center justify-center rounded-full bg-accent text-accent-foreground">1</span> : null}
            </Button>
          </PopoverTrigger>
          <PopoverContent align="end" collisionPadding={12} className="w-[min(var(--toolbar-popover-width),calc(100vw-2rem))] p-4">
            <div className="mb-3 flex items-center justify-between"><strong>Filtros</strong><Button type="button" variant="link" size="link" className="text-meta" onClick={() => setDraftStatus("all")}>Limpar</Button></div>
            <span className="mb-2 block text-label text-muted-foreground">Estado</span>
            ...
            <Button type="button" className="mt-4 w-full" onClick={() => { setStatus(draftStatus); dispatch(MARKETING_EVENTS.setStatusFilter, { status: draftStatus }); setOpen(false); }}>Aplicar</Button>
          </PopoverContent>
        </Popover>
      ) : null}
      <Button type="button" disabled={!ready} onClick={() => dispatch(MARKETING_EVENTS.create)}><Plus data-icon="inline-start" aria-hidden />{createLabel}</Button>
    </div>
  );
}
