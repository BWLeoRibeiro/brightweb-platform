          ...
          <AppSheetBody>
            <SheetSection title={dictionary.organizations.identity} editing={editing}>
              <FieldGroup className={`gap-0 px-0 py-1 ${editing ? "" : "divide-y divide-hairline"}`}>
                <Field className="gap-1.5 px-4 py-2"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.organizations.name}</FieldLabel><FieldContent><Input value={value.name ?? ""} onChange={(event) => setValue({ ...value, name: event.target.value })} placeholder={dictionary.organizations.namePlaceholder} disabled={!editing} className={`${controlClassName} mt-1.5`} /></FieldContent></Field>
                <Field className="gap-1.5 px-4 py-2"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.organizations.industry}</FieldLabel><FieldContent><select disabled={!editing} className={`${controlClassName} mt-1.5 text-foreground outline-none disabled:appearance-none`} value={value.industry ?? ""} onChange={(event) => setValue({ ...value, industry: event.target.value })}><option value="">{dictionary.organizations.selectIndustry}</option>{value.industry && !industries.includes(value.industry) ? <option value={value.industry}>{value.industry} (atual)</option> : null}{industries.map((industry) => <option key={industry} value={industry}>{industry}</option>)}</select></FieldContent></Field>
                <Field className="gap-1.5 px-4 py-2"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.organizations.website}</FieldLabel><FieldContent><Input value={value.website_url ?? ""} onChange={(event) => setValue({ ...value, website_url: event.target.value })} placeholder={dictionary.organizations.websitePlaceholder} disabled={!editing} className={`${controlClassName} mt-1.5`} /></FieldContent></Field>
                <Field className="gap-1.5 px-4 py-2"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.organizations.taxIdentifierLong}</FieldLabel><FieldContent><Input value={value.taxIdentifierValue ?? ""} onChange={(event) => setValue({ ...value, taxIdentifierValue: event.target.value })} placeholder={dictionary.organizations.taxIdentifierPlaceholder} inputMode="numeric" disabled={!editing} className={`${controlClassName} mt-1.5`} /></FieldContent></Field>
                <Field className="gap-1.5 px-4 py-2"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.organizations.address}</FieldLabel><FieldContent><Input value={value.address ?? ""} onChange={(event) => setValue({ ...value, address: event.target.value })} placeholder={dictionary.organizations.addressPlaceholder} disabled={!editing} className={`${controlClassName} mt-1.5`} /></FieldContent></Field>
              </FieldGroup>
            </SheetSection>
            <SheetSection title={dictionary.organizations.profile} editing={editing}>
              <FieldGroup className="grid grid-cols-2 gap-3 px-4 py-3">
                <Field><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.organizations.companySize}</FieldLabel><select disabled={!editing} className={`${controlClassName} mt-1.5 text-foreground outline-none disabled:appearance-none`} value={value.company_size ?? ""} onChange={(event) => setValue({ ...value, company_size: event.target.value })}><option value="">—</option>{companySizes.map((size) => <option key={size} value={size}>{size}</option>)}</select></Field>
                <Field><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.organizations.budgetRange}</FieldLabel><select disabled={!editing} className={`${controlClassName} mt-1.5 text-foreground outline-none disabled:appearance-none`} value={value.budget_range ?? ""} onChange={(event) => setValue({ ...value, budget_range: event.target.value })}><option value="">—</option>{budgetRanges.map((range) => <option key={range} value={range}>{range}</option>)}</select></Field>
              </FieldGroup>
            </SheetSection>
          </AppSheetBody>
