        ...
        <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col gap-0">
          ...
          <AppSheetBody>
            ...
            <SheetSection title={dictionary.contactDialog.pipeline} editing={mode !== "view"} bodyClassName="px-4 py-3">
              <p className={sheetFieldLabelClassName}>{dictionary.contactDialog.fields.status}</p>
              {mode === "view" ? <div className="mt-1.5">{activeStage ? <span className="tint-soft inline-flex items-center rounded-full border px-3 py-1 text-meta font-semibold leading-[var(--type-leading-130)]" style={tintStyle}>{activeStage.label}</span> : null}</div> : <div className="mt-1.5 flex flex-wrap items-center gap-1.5">{resolvedStages.map((stage) => <button key={stage.value} type="button" onClick={() => setValue({ ...value, status: stage.value })} className={`inline-flex shrink-0 items-center justify-center rounded-xl border px-2.5 py-2 text-meta font-semibold leading-[var(--type-leading-130)] normal-case tracking-normal transition ${value.status === stage.value ? "tint-soft shadow-sm" : "border-hairline bg-background text-foreground/50"}`} style={{ "--tint": `var(${stage.token})` } as CSSProperties}>{stage.label}</button>)}</div>}
            </SheetSection>
