            ...
            <SheetSection title={dictionary.contactDialog.information} editing={mode !== "view"}>
              <FieldGroup className={`gap-0 px-0 py-1 ${mode === "view" ? "divide-y divide-hairline" : ""}`}>
                <div className={`flex ${mode === "view" ? "divide-x divide-hairline" : "gap-3"}`}>
                  <div className="min-w-0 flex-1 px-4 py-2 sm:pr-5"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">Nome próprio</FieldLabel><Input value={value.firstName ?? ""} disabled={mode === "view"} onChange={(event) => setValue({ ...value, firstName: event.target.value })} placeholder={dictionary.contactDialog.placeholders.firstName} className={`${controlClassName} mt-1.5`} /></div>
                  <div className="min-w-0 flex-1 px-4 py-2 sm:pl-5"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">Apelido</FieldLabel><Input value={value.lastName ?? ""} disabled={mode === "view"} onChange={(event) => setValue({ ...value, lastName: event.target.value })} placeholder={dictionary.contactDialog.placeholders.lastName} className={`${controlClassName} mt-1.5`} /></div>
                </div>
                <Field className="gap-1.5 px-4 py-2"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.contactDialog.fields.email}</FieldLabel><FieldContent><Input type="email" value={value.email ?? ""} disabled={mode === "view"} onChange={(event) => setValue({ ...value, email: event.target.value })} placeholder={dictionary.contactDialog.placeholders.email} className={controlClassName} /></FieldContent></Field>
                <Field className="gap-1.5 px-4 py-2"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.contactDialog.fields.phone}</FieldLabel><FieldContent><PhoneInput value={value.phone ?? ""} disabled={mode === "view"} onChange={(phone) => setValue({ ...value, phone })} className={mode === "view" ? sheetViewControlClassName : "h-9 rounded-lg border border-[color:var(--sheet-edit-control-border)] bg-[color:var(--card)] px-2.5"} /></FieldContent></Field>
                <Field className="gap-1.5 px-4 py-2"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.contactDialog.fields.organization}</FieldLabel><FieldContent><select value={value.organizationId ?? ""} disabled={mode === "view"} onChange={(event) => setValue({ ...value, organizationId: event.target.value })} className={`${controlClassName} text-foreground outline-none disabled:pointer-events-none`}><option value="">{dictionary.contactDialog.placeholders.organization}</option>{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name ?? organization.id}</option>)}</select></FieldContent></Field>
                <Field className="gap-1.5 px-4 py-2"><FieldLabel className="text-[10px] uppercase tracking-wider text-foreground/40">{dictionary.contactDialog.fields.owner}</FieldLabel><FieldContent>{mode === "view" ? <p className="text-sm text-foreground/75">{ownerLabel}</p> : <select value={value.ownerId ?? ""} onChange={(event) => setValue({ ...value, ownerId: event.target.value })} className={`${sheetEditControlClassName} text-foreground outline-none`}><option value="">{dictionary.contactDialog.placeholders.owner}</option>{owners.map((owner) => <option key={owner.id} value={owner.id}>{owner.label} ({owner.role === "admin" ? "Admin" : "Staff"})</option>)}</select>}</FieldContent></Field>
              </FieldGroup>
            </SheetSection>

            <SheetSection title={dictionary.contactDialog.pipeline} editing={mode !== "view"} bodyClassName="px-4 py-3">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground/40">{dictionary.contactDialog.fields.status}</p>
