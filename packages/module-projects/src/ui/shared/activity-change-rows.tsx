        ...
        <li key={change.key} className="portal-micro flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5 font-normal">
          ...
          {change.from || change.to ? (
            <span className="inline-flex items-baseline gap-1.5 text-[color:var(--muted-foreground)]">
              {change.from ? <span className="line-through opacity-70">{change.from}</span> : null}
              {change.from && change.to ? <span aria-hidden>→</span> : null}
              {change.to ? <span className="font-semibold text-[color:var(--foreground)]">{change.to}</span> : null}
            </span>
          ) : <span className="text-[color:var(--muted-foreground)]">{defaultProjectsUiDictionary.detail.updatedLowercase}</span>}
        </li>
      ))}
    </ul>
  );
