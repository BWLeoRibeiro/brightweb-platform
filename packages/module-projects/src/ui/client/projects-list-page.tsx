      ...
      <div className="panel-inner space-y-6">
        <header className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            <h1 className="text-heading-2 font-semibold leading-tight">{dictionary.title}</h1>
            <p className="text-body text-muted-foreground">{dictionary.description}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link
              href="/account"
              className="inline-flex items-center gap-2 rounded-lg border border-border/70 bg-background/70 px-3 py-2 text-body font-semibold transition-colors hover:bg-background motion-reduce:transition-none"
            >
            ...
            {isStaff ? (
              <Link
                href="/projects"
                className="inline-flex items-center gap-2 rounded-lg border border-primary/30 bg-primary/10 px-3 py-2 text-body font-semibold text-primary transition-colors hover:bg-primary/15 motion-reduce:transition-none"
              >
        ...
        {!projects.schemaMissing && !projects.error && projects.items.length === 0 ? (
          <div className="rounded-xl border border-border/60 bg-background/60 px-4 py-10 text-center">
            ...
            <p className="text-body text-muted-foreground">{dictionary.empty}</p>
          </div>
        ) : null}
